...
function convertTable(from, to, value) {
    ...
    var pos = 0;
    value = value.replace(",", ".");
    var is_num = (value == parseFloat(value));
    for (var i = 0; i < from_array.length; i++) {
        if (value.toUpperCase() == from_array[i].toUpperCase()) {
            pos = i + 1;
            pre = '';
            break;
        } else if (is_num && from_array[i] == parseFloat(from_array[i])) {
            if (dif == 0) {
                dif = Math.abs(from_array[i] - value);
                pre = '~';
                pos = i + 1;
            } else if (Math.abs(from_array[i] - value) <= dif) {
                dif = Math.abs(from_array[i] - value);
                pos = i + 1;
                pre = '~';
            }
    ...
    if (pos > 0) {
        return pre + to_array[pos - 1]
    }

    return'-';
}

function convertPaper(from, to, constant, decimals) {
    if (to == '0')
        return'-';
    from = from.replace(",", ".");
    to = to.replace(",", ".");
    var split_array = from.split("x");
    var from_a = parseFloat(split_array[0]);
    var from_b = parseFloat(split_array[1]);
    if (to == 'P') {
        var from_c = from_a * from_b;
        var split_array = constant.split("x");
        var to_a = parseFloat(split_array[0]);
        var to_b = parseFloat(split_array[1]);
        var to_c = to_a * to_b;
        var result = to_c / from_c;
        var tmp = result;
        result = myRound(result, decimals);
        tmpStr = '' + result;
        if (result == 0 || tmpStr.length > 12 || tmpStr.indexOf('e') > 0)
            result = myRoundForced(tmp, decimals);
        return'\u00D7' + result;
    } else {
        var to_a = parseFloat(to) * constant;
        var a = from_a * to_a;
        var b = from_b * to_a;
        var tmp = a;
        a = myRound(a, decimals)

        tmpStr = '' + a;
        if (a == 0 || tmpStr.length > 12 || tmpStr.indexOf('e') > 0)
            a = myRoundForced(tmp, decimals);
        a = formatResult(a);
        var tmp = b;
        b = myRound(b, decimals)

        tmpStr = '' + b;
        if (b == 0 || tmpStr.length > 12 || tmpStr.indexOf('e') > 0)
            b = myRoundForced(tmp, decimals);
        b = formatResult(b);
        return a + ' x ' + b;
    }
}

function convertCurrency(amount, from, to, div, include_short, decimals) {
    var done = false;
    var split_array = from.split("|");
    var from_constant_str = split_array[0];
    var date_from = split_array[1];
    var from_unit = split_array[2];
    var split_array = to.split("|");
    var to_constant_str = split_array[0];
    var date_to = split_array[1];
    ...
    if (isNaN(result))
        ...
        new_str = formatResult(result) + ' ' + to_unit;
    else
        new_str = formatResult(result);
    var dateDiv;
    if (!div) {
        div = replaceAll(to_unit, ' ', '-');
        div = replaceAll(div, '/', '-');
        div = 'value_' + div;
        document.getElementById('date' + div.substring(5, 9)).value = formatDate(date_to, 2);
    } else {
        new_str = "<b>" + new_str + "</b>";
        dateDiv = 'date';
        setInnerHTML('date', formatDate(date_to, 2));
    }
...
function convert(amount, from, to, div, include_short, decimals) {
    ...
    var to_unit = split_array[2];
    var new_str;
    if (to_constant_str == 'T') {
        new_str = convertTable(from_constant_str_extra, to_constant_str_extra, amount);
    } else if (to_constant_str == 'P') {
        new_str = convertPaper(to_constant_str_extra, from_constant_str, from_constant_str_extra, decimals);
    } else {
        amount = amount.replace(",", ".");
        to_constant_str = to_constant_str.replace(",", ".");
        from_constant_str = from_constant_str.replace(",", ".");
        ...
        var m;
        if (a2 != -888 && b2 == -888) {
            split_array = from_constant_str.split(":");
            b1 = parseFloat(split_array[0]);
            b2 = parseFloat(split_array[1]);
            result = Math.pow(10, amount / b1) / b2 * a1;
        } else if (a2 == -888 && b2 != -888) {
            if (amount == 0) {
                result = '\u221e';
                done = true;
            } else if (amount > 0) {
                split_array = to_constant_str.split(":");
                a1 = parseFloat(split_array[0]);
                a2 = parseFloat(split_array[1]);
                result = a1 * Math.log(a2 * amount / b1) * Math.LOG10E;
            }
        } else if (a2 == -999 && b2 == -999) {
            k = a1 / b1;
            result = amount * k;
...
function formatResult(str) {
    ...
    var ret = '';
    if (parts[0] + 0 == parts[0] || parts[0].indexOf("e") == -1) {
        ret = addSeparators(str);
    } else {
        var split_array = str.split("e");
        if (parseFloat(split_array[0]) != 1) {
            var ret = addSeparators(parseFloat(split_array[0])) + '\u00D7' + '10<sup>' + parseFloat(split_array[1]) + '</sup>';
        } else {
            var ret = '10<sup>' + parseFloat(split_array[1]) + '</sup>';
        }
...
function initRoman() {
    for (var i = 0; i < initRoman.arguments.length; i++)
        this[i + 1] = initRoman.arguments[i];
}

function toRoman(number) {
    if (number <= 0 || isNaN(number) || number >= 4000000)
        return'-';
...
function toEnglish(amount) {
    ...
    for (var i = 0; i < amount_Digits.length; i++) {
        if (amount_Digits[i] != '-')
            amountDigitValues[i] = parseInt(amount_Digits[i]);
            ...
            case 1:
                switch (pos) {
                    ...
                    case 4:
                        ...
                        break;
                    case 1:
                        if (numstr.substring(len - pos - 1) == "01") {
                            msg += "s";
                        }
...
function convertNumerals(amount, from, to, div, include_short, decimals) {
    var split_array = from.split("|");
    var from_constant_str = split_array[0];
    var from_decimal;
    var from_unit = split_array[2];
...
function cityClick(str, from) {
    showHideClock3(true, from);
    changeUTC(str, true, true, true, true, true, true, from);
    return false;
}

function changeUTC(str, boolMap, boolClock, boolCityDrop, boolZoneDrop, boolUTCDrop, boolCities, from) {
    var split_array = str.split("|");
    var utc = split_array[0];
    var city = split_array[1];
    var dst = split_array[2];
    var found = false;
    if (boolMap && from == 1) {
        var flashMap;
        if (navigator.appName.indexOf("Microsoft") != -1) {
            flashMap = window.flashObject;
        } else {
            flashMap = window.document.flashObject;
        }
        ...
        if (dst != '') {
            ...
            if (navigator.appName.indexOf("Microsoft") != -1) {
                switch (from) {
                    case 3:
                    case 4:
                        flashClock = window.flashObject3;
                        break;
                    default:
                        flashClock = window.flashObject2;
                        break;
                    }
            } else {
                switch (from) {
                    case 3:
                    case 4:
                        flashClock = window.document.flashObject3;
                        break;
                    default:
                        flashClock = window.document.flashObject2;
                        break;
                    }
            }

            if (from == 1) {
                setInnerHTML('city', city);
            }

            else if (from != 4 && from != 5) {
                document.getElementById('cityfield' + from).value = city;
            }
    ...
    if (boolUTCDrop && from == 1) {
        SelectObject = document.getElementById('byutc');
        found = false;
        for (index = 0; index < SelectObject.length; index++) {
            if (SelectObject[index].value == utc) {
    ...
    if (boolCities && from == 1) {
        for (index = 0; index < cityDiv.length; index++) {
            if (cityDiv[index] == utc)
                showDiv('cities' + index);
            else
                hideDiv('cities' + index);
        }
    }
}

function cityChange(text, from, idStr, isExtended) {
    if (text.value.length == 0) {
        moveIt('citylist', 1, 1, 1, '');
        showHideClock3(true, from);
    } else {
        var bgr = Array('#FFF4B8', '#FFD600');
        var j = 0;
        var displayed = 0;
        var h = 0;
        var h_row = 14;
        var searchStr;
        var matchStr = stripAccents(text.value.toLowerCase());
        var displayStr = '<ul>';
        var extra = 'show';
        var a_end = '</a>';
        var a_start;
        for (var i = 0; i < city.length; i++) {
            if (city[i] != null) {
                searchStr = stripAccents(city[i][0].toLowerCase());
                if (searchStr.match(matchStr) != null) {
                    if (displayed++ == 20)
                        extra = extra + ', scroll';
                    if (displayed < 20)
                        h += h_row;
                    a_start = '<a href="/www/convertworld/en/time_zone/' + escape(city[i][0]) + '.html" onclick=\"moveIt(\'citylist\', 1, 1, 1, \'\'); return cityClick(\'' + city[i][1] + '|' + city[i][0].replace('\'', '\\\'') + '|' + city[i][2] + '\',' + from + ');\">';
                    displayStr = displayStr + '<div class="unit_' + (displayed % 2 ? 'even' : 'odd') + '_color">' + a_start + city[i][0] + a_end + '</div>\n';
                }
            }
        }

        if (displayed == 0) {
            displayStr = displayStr + '<div style="float: left; width: 100%; white-space: nowrap; padding-left: 10px" class="unit_' + (displayed % 2 ? 'even' : 'odd') + '_color">&nbsp;</div>\n';
            h = h_row;
        }

        displayStr = displayStr + '</ul>';
        setInnerHTML('citylist', displayStr);
        var pos = $('input#' + idStr).offset();
        if (isExtended) {
            moveIt('citylist', pos.top - 92, pos.left - 160, h, extra);
        } else {
            if (t)
                moveIt('citylist', pos.top - 114, 12, h, extra);
            else
                moveIt('citylist', pos.top + 20, pos.left, h, extra);
        }
...
function moveIt(id, mvTop, mvLeft, h, dowhat) {
    e = document.getElementById(id);
    e.style.top = mvTop + 'px';
    e.style.left = mvLeft + 'px';
    if (h > 0) {
        e.style.height = h + 'px';
    }

    if (dowhat.match('show') != null)
        e.style.display = 'block';
    else
        e.style.display = 'none';
    if (dowhat.match('scroll') != null)
...
function showHideClock3(show, from) {
    if (from == 1)
        return;
    e = document.getElementById('flashObject3');
    if (show) {
        e.width = "150px";
        e.height = "44px";
    } else if (!show && from == 2) {
...
function converterChange(converter) {
    var from = document.forms['conv_' + converter].unit_from.selectedIndex;
    var to = document.forms['conv_' + converter].unit_to.selectedIndex;
    document.forms['conv_' + converter].unit_from.selectedIndex = to;
    document.forms['conv_' + converter].unit_to.selectedIndex = from;
}
...
$(document).ready(function() {
    $('.tabs a').click(function() {
        ...
        $('#content').hide();
        $('div#loading').show();
        $.ajax({type: 'POST', url: $(this).attr('href'), data: 'ajax=1', async: true, success: function(d) {
                c = d;
                r = false;
                $('div#header .tabs a').height(23);
                o.height(24);
                $('#header .search :input:visible:enabled:first').focus();
                if (document.getElementById)
                    document.getElementById('uri').value = 'http://' + h + o.attr('href');
                if (o.attr('href') == l) {
                    $('#menu img.plus').fadeIn(500);
                } else {
                    $('#menu img.plus').fadeOut(500);
                }
    ...
    $('div.login a#in').click(function() {
        if ($('#jq_login').is(':visible')) {
            $('#jq_login').fadeOut(500);
            $('div.register').fadeIn(500);
        } else {
            $('div.register').fadeOut(500);
            $('#jq_login').fadeIn(500);
        }
...
function addMini(c) {
    ...
    for (var i = 0; i < $('.groupItem').length; i++) {
        e = $(a[i]).attr('id');
        if (e == c) {
            $('#content #' + c).fadeOut(300);
            $('#content #' + c).fadeIn(300);
            return;
        }
    }

    var m = $('#sort1');
    if (m.children('.groupItem').length > $('#sort2').children('.groupItem').length)
        m = $('#sort2');
    if (m.children('.groupItem').length > $('#sort3').children('.groupItem').length)
        m = $('#sort3');
    m.prepend('<div id="' + c + '" class="groupItem"><div class="dotted"><img src="' + p + '/static/images/loading.gif" alt="Loading..." style="margin: 102px 0 0 81px" /></div></div>');
    minisUpdated();
    loadIt(c, m, true, 'on');
}

function loadIt(c, m, dots, dyn) {
    $.ajax({type: 'GET', url: p + '/ajax/mini.php', data: 'c=' + c + '&l=' + la + '&dyn=' + dyn, async: true, success: function(d) {
            if (dots) {
                m.children('#' + c).html('<div class="dotted">' + d + '</div>');
            } else {
                m.children('#' + c).html(d);
            }
...
function menuArrow(slider, button) {
    if (!document.getElementById)
        return;
    if ($(slider).is(':visible')) {
        document.getElementById(button).src = aR.src;
    } else {
        document.getElementById(button).src = aD.src;
    }
}

function menuToggle(slider, button) {
    $(slider).slideToggle(200, menuArrow(slider, button));
}
...
function savePos(c1, c2, c3) {
    $.ajax({type: 'GET', url: p + '/ajax/settings.php', data: 'c1=' + c1 + '&c2=' + c2 + '&c3=' + c3, async: true, success: function(d) {
            debug(d);
        }});
}
...
function minisUpdated() {
    if ((new Date().getTime()) < (tim + 200))
        return;
    tim = new Date().getTime();
    clearTimeout(ti);
    ti = setTimeout('savePos(\'' + $('#sort1').sortable('toArray') + '\',\'' + $('#sort2').sortable('toArray') + '\',\'' + $('#sort3').sortable('toArray') + '\')', 3000);
}

function showPopup(c) {
    window.open(p + '/converters/popup.php?l=' + la + '&c=' + c + '&t=' + t, c, "status=0,toolbar=0,location=0,menubar=0,directories=0,resizable=0,scrollbars=0,height=250,width=200");
}

function animateMinis() {
    var els = ['#sort1', '#sort2', '#sort3'];
    var $els = $(els.toString());
    $els.sortable('refresh');
    $els.sortable('disable');
    $('div#content .groupItem').hover(function() {
        $(this).children('div').children('div').children('.tools').children('div').fadeIn(100);
    }, function() {
        $(this).children('div').children('div').children('.tools').children('div').fadeOut(100);
    });
    $('div#content .groupItem .move').hover(function() {
        $els.sortable('enable');
    }, function() {
        $els.sortable('disable');
    });
    $('div#content .groupItem .close').click(function() {
        var link = $(this), item = link.parents('.groupItem');
        item.remove();
        minisUpdated();
    });
    $('div#content .groupItem .new').click(function() {
        var link = $(this), item = link.parents('.groupItem');
        showPopup(item.attr('id'));
    });
}

function addfav(url, title, msgNetsc) {
    if (window.sidebar) {
